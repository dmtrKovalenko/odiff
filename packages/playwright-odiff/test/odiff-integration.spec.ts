...
test.describe("toHaveScreenshotOdiff - Integration Tests", () => {
  ...
  test("should take screenshot with auto-generated name", async ({ page }) => {
  ...
  test("should take screenshot with options only (no name)", async ({
    page,
  }) => {
    await page.goto(testPagePath);
    await page.waitForLoadState("networkidle");

    // Pass options without name - should auto-generate name
    await expect(page).toHaveScreenshotOdiff({
      threshold: 0.1,
      fullPage: true,
    });
  ...
  test("should take and compare screenshot with custom threshold", async ({
    page,
  }) => {
    await page.goto(testPagePath);
    await page.waitForLoadState("networkidle");
  ...
  test("should take screenshot of specific feature section", async ({
    page,
  }) => {
    await page.goto(testPagePath);
    await page.waitForLoadState("networkidle");
  ...
  test("should compare individual feature cards", async ({ page }) => {
    ...
    for (let i = 0; i < features.length; i++) {
      await expect(features[i]).toHaveScreenshotOdiff(
        `test-page-feature-${i + 1}.png`,
      );
    }
  });
