...
pub fn runServerMode(allocator: std.mem.Allocator) !void {
    ...
    while (true) {
        ...
        const images = odiff.io.loadTwoImages(allocator, base_path, compare_path, strategy) catch |err| {
            var msg_buf: [512]u8 = undefined;
            const msg = switch (err) {
                error.ImageNotLoaded, error.UnsupportedFormat => try std.fmt.bufPrint(&msg_buf, "Could not load images: {s} or {s}", .{ base_path, compare_path }),
                else => try std.fmt.bufPrint(&msg_buf, "Failed to load images: {}", .{err}),
            };
            try response_writer.writeError(request_id, msg);
            continue;
        };
